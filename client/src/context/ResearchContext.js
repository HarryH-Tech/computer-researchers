...
const ResearchProvider = ({ children }) => {
    ...
    const getAllResearchers = () => {
        axios_1.default
            .get(process.env.REACT_APP_RESEARCHER_CRUD_BASE_URL + '/get_all_researchers')
            .then((res) => {
    ...
    const getResearcher = (researcherName) => __awaiter(void 0, void 0, void 0, function* () {
        const response = yield axios_1.default.get(process.env.REACT_APP_RESEARCHER_CRUD_BASE_URL +
    ...
    const saveResearcher = (researcher) => {
        axios_1.default
            .post(process.env.REACT_APP_RESEARCHER_CRUD_BASE_URL + '/create_researcher', {
            researcher,
        })
            .then((res) => {
            if (res.status === 200) {
                ...
                setErrorMessage("Sorry, we couldn't add that right now. Please try again later.");
            }
        });
    };
    // DELETE RESEARCHER
    const deleteResearcher = (id) => {
        axios_1.default
            .delete(process.env.REACT_APP_RESEARCHER_CRUD_BASE_URL + '/delete_researcher', {
            data: {
                id,
            },
        })
            ...
            if (res.status === 200) {
                ...
                setErrorMessage("Sorry, we couldn't delete that right now. Please try again later.");
            }
        })
            .catch((err) => {
            setErrorMessage(err.message);
    ...
    const deleteResearchers = (researchers) => {
        ...
        axios_1.default
            .delete(process.env.REACT_APP_RESEARCHER_CRUD_BASE_URL + '/delete_researchers', {
            data: {
                ids: researchers,
            },
        })
    ...
    const editResearcher = () => {
        axios_1.default
            .put(process.env.REACT_APP_RESEARCHER_CRUD_BASE_URL + '/edit_researcher', {
            researcher,
        })
            .then((res) => {
            if (res.status === 200) {
                ...
                setErrorMessage("Sorry, we couldn't edit that right now. Please try again later.");
            }
        })
            .catch((err) => {
            setErrorMessage(err.message);
    ...
    return (react_1.default.createElement(exports.ResearchContext.Provider, { value: {
            researchers,
            researcher,
            errorMessage,
            loading,
